...
Keempat halaman memakai ISR: `export const revalidate = 1800`. Halaman
disajikan sebagai HTML statis dari CDN dan dibangun ulang tiap 30 menit,
selaras dengan TTL cache data di `lib/aggregate.js`.

Sebelumnya keempatnya memakai `dynamic = "force-dynamic"`. Itu tidak
memberi data yang lebih baru, karena datanya sudah di-cache 30 menit
lewat `unstable_cache`, tapi memaksa satu perjalanan bolak-balik ke
fungsi server pada tiap klik navigasi. Di paket gratis, klik antar
halaman terasa tersendat karena menunggu fungsi yang sedang dingin.

Ada dua lapis cache, dan `/api/refresh` membatalkan keduanya: tag `jobs`
untuk data, dan path tiap halaman untuk HTML yang sudah dibangun.

Satu hal yang perlu diketahui: kalau semua sumber kebetulan gagal
dihubungi tepat saat halaman dibangun ulang, keadaan kosong itu bisa
tersimpan sampai 30 menit. Untuk memaksa pembangunan ulang lebih awal:
